...
function NIRTab() {
  ...
  const updateItem = (idx, field, value) => {
    ...
    if (field === "productId" && value && value !== "new") {
      ...
      if (product) {
        ...
        items[idx].price = product.price; // Auto-fill price but still editable
        items[idx].isNew = false;
      }
    }
    
    // If selecting "new", enable manual entry and generate next code
    if (field === "productId" && value === "new") {
      items[idx].isNew = true;
      items[idx].productId = "";
      items[idx].productName = "";
      
      // Generate next available COD M.P (max code + 1)
      const maxCode = products.reduce((max, p) => {
        const code = parseInt(p.code) || 0;
        return code > max ? code : max;
      }, 0);
      items[idx].productCode = String(maxCode + 1).padStart(4, '0');
    }
  ...
  const submit = async (e) => {
    ...
    if (!form.supplierId) {
      alert("Vă rugăm completați furnizorul");
      return;
    }
    ...
    // Create NIR (number will be auto-generated by backend)
    await fetch("/api/nir", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        paidBase: Number(form.paidBase) || 0,
        paidVat: Number(form.paidVat) || 0,
  ...
  const deleteNir = async (id) => {
    if (!confirm("Sunteți sigur că doriți să ștergeți acest NIR? Stocul va fi actualizat.")) {
      return;
    }
    
    try {
      await fetch(`/api/nir/${id}`, { method: "DELETE" });
      load();
    } catch (err) {
      console.error("Error deleting NIR:", err);
      alert("Eroare la ștergerea NIR-ului");
    }
  };
  ...
  const calculateValidation = () => {
    ...
    for (const item of form.items) {
      if (item.quantity && item.price) {
        ...
        base += value;
        vat += value * (Number(item.vatRate) / 100);
      }
    }
    return { base: base.toFixed(2), vat: vat.toFixed(2) };
  };
  ...
  return (
    <div>
      <form onSubmit={submit} className="mb-6 bg-gray-800 rounded-xl p-4">
        ...
        {/* Header fields - Supplier, Auto NIR number, Date, Invoice */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
          <div>
            <label className="block text-xs text-gray-400 mb-1">Furnizor *</label>
            <select 
              ...
              required
            >
              <option value="">Selectați furnizor</option>
              {suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          </div>
          
          <div>
            <label className="block text-xs text-gray-400 mb-1">Nr. NIR (auto)</label>
            <input 
              type="text"
              value={`NIR-${String(nirs.length + 1).padStart(4, '0')}`}
              readOnly
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Dată NIR *</label>
            <input 
              type="date"
              value={form.date}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Factură nr.</label>
            <input 
              type="text"
        ...
        <div className="bg-gray-700/30 rounded-lg p-4 mb-4">
          <h4 className="text-sm font-semibold text-amber-400 mb-3">Validare și Sume Plătite</h4>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            {/* Validation - Base */}
            <div>
              <label className="block text-xs text-gray-400 mb-1">Validare – Bază (Lei)</label>
              <input 
                type="text"
                value={calculateValidation().base}
                readOnly
                className="w-full bg-gray-600 border border-gray-600 rounded-lg px-3 py-2 text-sm text-green-400 font-medium"
              />
            </div>
            
            {/* Validation - VAT */}
            <div>
              <label className="block text-xs text-gray-400 mb-1">Validare – TVA (Lei)</label>
              <input 
                type="text"
                value={calculateValidation().vat}
                readOnly
                className="w-full bg-gray-600 border border-gray-600 rounded-lg px-3 py-2 text-sm text-green-400 font-medium"
              />
            </div>
            ...
            {/* Paid - Base */}
            <div>
              <label className="block text-xs text-gray-400 mb-1">Sume plătite – Bază (Lei)</label>
              <input 
                type="number"
                step="0.01"
                placeholder="0.00"
                value={form.paidBase}
                onChange={(e) => setForm({ ...form, paidBase: e.target.value })}
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm"
              />
            </div>
            ...
            {/* Paid - VAT */}
            <div>
              <label className="block text-xs text-gray-400 mb-1">Sume plătite – TVA (Lei)</label>
              <input 
                type="number"
                step="0.01"
                placeholder="0.00"
                value={form.paidVat}
                onChange={(e) => setForm({ ...form, paidVat: e.target.value })}
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm"
              />
            </div>
          </div>
            ...
            <div className="flex justify-between items-start mb-3">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className="text-gray-400 text-sm">{nir.supplier.name}</span>
                  <span className="text-gray-500 text-xs">{new Date(nir.date).toLocaleDateString("ro-RO")}</span>
                </div>
                {nir.invoiceNumber && (
                  <div className="text-gray-400 text-xs">Factură nr.: {nir.invoiceNumber}</div>
                )}
              </div>
              <div className="flex gap-2">
                <button 
                ...
                <button 
                  onClick={() => deleteNir(nir.id)}
                  className="bg-red-600 hover:bg-red-500 px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1"
                >
                  🗑️ Șterge
                </button>
              </div>
